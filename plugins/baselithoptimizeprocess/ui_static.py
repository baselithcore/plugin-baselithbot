...
def mount_dashboard_ui(router: Any) -> None:
    ...
    Args:
        router: The plugin's ``APIRouter``. Routes ``GET /ui/`` and
            ``GET /ui/{path:path}`` are added; a missing build degrades to a
    ...
    async def _ui_root() -> Any:
        if not index_file.exists():
            return HTMLResponse(
                "<h1>BaselithOptimizeProcess</h1><p>Dashboard not built. "
                "Run <code>npm install &amp;&amp; npm run build</code> under "
                "<code>ui/</code>.</p>",
                status_code=200,
            )
