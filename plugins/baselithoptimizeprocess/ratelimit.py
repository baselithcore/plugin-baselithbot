"""Opt-in per-tenant rate limiting for the BOP API (fixed-window counter).
...
expensive/abusable mutating endpoints (ingest, optimize). In-memory and
per-process — adequate for a single replica; a multi-replica deployment should
front BOP with a shared gateway limiter.
"""
...
# Stop the bucket map from growing without bound under churny tenant ids.
_MAX_BUCKETS = 10_000
