"""Optional RBAC for the BOP routers.

Role-based access control is **opt-in** and fully decoupled from the auth
plugin: BOP carries no hard dependency on it. By default (``auth_enabled``
False) every endpoint is open — identical to the plugin's pre-RBAC behaviour.
Set the plugin config flag ``auth_enabled: true`` to gate endpoints; that path
requires :mod:`plugins.auth` to be loaded (it provides ``require_roles``). If
the flag is on but the auth plugin is absent, endpoints fall back to a warned
no-op so the service still runs rather than hard-failing.

Three privilege tiers map onto :class:`core.auth.AuthRole`:

    * ``VIEWER_ROLES`` — read-only surface (list/get/snapshots/stream/health).
    * ``EDITOR_ROLES`` — mutating surface (register/import/ingest/optimize,
      rule and process management). Excludes GUEST.
    * ``APPROVER_ROLES`` — the HITL decision surface (approve / reject a
      proposal). Restricted to ADMIN/SERVICE — the privileged act in the
      human-in-the-loop contract, deliberately narrower than ``EDITOR_ROLES``.
...
try:
    ...
    logger.debug("BOP auth gating disabled: %s", exc)
    _require_roles = None
    VIEWER_ROLES = ()
    EDITOR_ROLES = ()
    APPROVER_ROLES = ()
...
def _gate(roles: tuple[Any, ...], enabled: bool) -> Callable[..., Any]:
    ...
    if not _AUTH_AVAILABLE or _require_roles is None:
        if not _AUTH_WARNING_EMITTED:
            logger.warning(
                "BOP auth_enabled=true but the auth plugin is not available; "
...
def require_editor(enabled: bool = False) -> Callable[..., Any]:
    """Dependency for mutating endpoints (RBAC applied only when enabled)."""
    return _gate(EDITOR_ROLES, enabled)


def require_approver(enabled: bool = False) -> Callable[..., Any]:
    """Dependency for the proposal decision surface (RBAC only when enabled)."""
