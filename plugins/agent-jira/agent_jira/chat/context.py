...
def build_context_and_sources(
    ...
    *,
    ...
    # Rileva se i risultati provengono da pochi documenti con molti chunk
    # (tipico di file tabulari/Excel). In quel caso rilassiamo il vincolo
    # di adiacenza e aumentiamo i chunk per documento.
    unique_docs: set[str] = set()
    for hit, _ in ranked_hits:
        payload = getattr(hit, "payload", None) or {}
        doc_raw = payload.get("document_id")
        if doc_raw is not None:
            unique_docs.add(str(doc_raw))
    few_documents = len(unique_docs) <= 2
    effective_max_chunks = final_top_k if few_documents else max_chunks_per_document
    ...
    for hit, score in ranked_hits:
        ...
        if chunk_index_int is not None:
            ...
            if chunk_index_int in existing_indices:
                continue
            # Vincolo di adiacenza: per documenti tabulari (pochi doc, molti chunk)
            # i chunk sono record indipendenti → il reranker li ha già ordinati per
            # rilevanza, non serve vincolare all'adiacenza posizionale.
            if not few_documents:
                primary_index = entry.get("primary_index")
                if (
                    primary_index is not None
                    and abs(chunk_index_int - primary_index) > 2
                ):
                    continue

        if len(entry["payloads"]) >= effective_max_chunks:
            continue
    ...
    # Il fallback forzato è stato rimosso per garantire che vengano usati solo documenti pertinenti.
    # Se top_chunks è vuoto, il contesto sarà una stringa vuota.
    ...
    for doc_info in doc_sources:
        doc_key = doc_info.get("document_id")
        if not isinstance(doc_key, str):
            continue

        stats = doc_stats.get(doc_key)
        if not stats:
            continue
