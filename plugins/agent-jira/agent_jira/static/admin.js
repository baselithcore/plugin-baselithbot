...
function summarizeSources(sources) {
  ...
  if (!labels.length) {
    const fallback = tooltip || 'Nessuna fonte disponibile';
...
async function reindexDocs() {
  try {
    ...
    const data = await res.json();
    alert('Reindicizzazione completata ✅ Nuovi file: ' + data.new_files_indexed);
    await loadData();
  } catch (err) {
    console.error('[admin] Failed to reindex documents:', err);
    alert('❌ Reindicizzazione fallita: ' + (err && err.message ? err.message : err));
  }
}
