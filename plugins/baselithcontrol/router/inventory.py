...
from ..api_models import EmbedSurface, InventoryView, WhoamiView
from ..service import get_aggregator
from ._guards import current_principal, is_admin, read_guard, system_visibility


def _whoami_sync(user: AuthUser) -> WhoamiView:
    """Build the caller's identity view (sync — run off the event loop)."""
    email = user.email
...
def build_inventory_router() -> APIRouter:
    ...
    @router.get("/me", response_model=WhoamiView)
    async def whoami(user: AuthUser = Depends(current_principal)) -> WhoamiView:
        """Identity + capabilities of the caller, so the UI can gate actions."""
        # DB enrichment + RBAC admin check are sync SQL — keep them off the loop.
        return await asyncio.to_thread(_whoami_sync, user)
    ...
    async def inventory(
        ...
        """
        admin = await asyncio.to_thread(is_admin, user)
        system_allow = (
            None if admin else await asyncio.to_thread(system_visibility, user)
        )
        return get_aggregator(request.app).inventory(
            include_disabled=admin, system_allow=system_allow
        )
    ...
    async def ui_registry(
        ...
        admin = await asyncio.to_thread(is_admin, user)
        if admin:
            allow, system_allow = None, None
        else:
            allow = await asyncio.to_thread(_tab_access_predicate, user)
            system_allow = await asyncio.to_thread(system_visibility, user)
