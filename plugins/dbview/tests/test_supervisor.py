...
These tests deliberately avoid spawning a real Node process — that path
is covered by the integration suite that requires ``node`` on PATH. The
unit layer focuses on:
...
def test_ensure_runtime_available_raises_when_node_missing(tmp_path: Path):
    ...
    with patch("plugins.dbview.supervisor.shutil.which", return_value=None):
