...
const DEFAULT_REMOTE_MODEL: Record<Exclude<LlmProvider, 'ollama'>, string> = {
  openai: process.env.OPENAI_MODEL ?? 'gpt-4o-mini',
  anthropic: process.env.ANTHROPIC_MODEL ?? 'claude-sonnet-4-6',
};

export function defaultModelFor(provider: LlmProvider, kind: DialectKind): string {
  if (provider === 'ollama') return DEFAULT_OLLAMA_MODEL_BY_KIND[kind];
 ...
 */
class GenericRemoteAdapter implements LlmAdapter {
...
export function createLlmAdapter(
  ...
  switch (provider) {
    case 'ollama': {
      ...
      if (SQLCODER_FAMILY.test(model)) return new SqlcoderAdapter(baseUrl);
 ...
 */
export function createChatAdapter(provider: LlmProvider, auth: LlmAdapterAuth = {}): LlmAdapter {
  switch (provider) {
    case 'ollama':
...
const DEFAULT_EXPLAIN_MODEL: Record<LlmProvider, string> = {
...
export function defaultExplainModel(provider: LlmProvider): string {
