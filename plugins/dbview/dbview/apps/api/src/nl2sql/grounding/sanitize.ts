...
export function sanitize(query: string, opts: SanitizeOptions): SanitizeResult {
  let out = query;
  const fixes: string[] = [];
 ...
 */
function collapseMultiDots(sql: string, fixes: string[]): string {
  if (!/\.{2,}/.test(sql)) return sql;
 ...
 */
function stripOrphanDots(sql: string, fixes: string[]): string {
  let out = sql;
  let changed = false;
  // `<ws|,|(> . <ws|,|)|;|end>` — standalone orphan dot.
  out = out.replace(/([\s,(])\.(?=[\s,);]|$)/g, (_, pre: string) => {
    changed = true;
    return pre;
  });
  // Leading dot on an identifier when preceded by ws/comma/paren: `, .Name`.
  out = out.replace(/(^|[\s,(])\.([A-Za-z_])/g, (_, pre: string, c: string) => {
    changed = true;
    return `${pre}${c}`;
  });
  ...
  if (changed) fixes.push('stripped orphan dot tokens');
  return out;
}

function collapseSpacedDots(sql: string, fixes: string[]): string {
...
function stripTrailingDot(sql: string, fixes: string[]): string {
  const re = /\s*\.\s*;?\s*$/;
  if (!re.test(sql)) return sql;
...
function fixFetchNext(sql: string, rowLimit: number, fixes: string[]): string {
  return sql.replace(SQL_FETCH_NEXT_PLACEHOLDER, (match, token: string) => {
    if (/^\d+$/.test(token)) return match;
...
function stripTrailingEllipsis(sql: string, fixes: string[]): string {
  if (!TRAILING_ELLIPSIS.test(sql)) return sql;
...
function stripBogusSchemaPrefixes(
  sql: string,
  knownTables: Set<string>,
  stripAll: boolean,
  fixes: string[],
  ...
  return sql.replace(pattern, (match, prefix: string, table: string) => {
    const qualified = `${prefix}.${table}`;
    if (knownTables.has(qualified)) return match;
    const lowerTables = new Set([...knownTables].map((t) => t.toLowerCase()));
    if (!lowerTables.has(table.toLowerCase())) return match;
    if (!stripAll && knownTables.has(prefix)) return match;
 ...
 */
function enforceNotNullOnRanking(sql: string, fixes: string[]): string {
  if (/\(\s*SELECT\b/i.test(sql)) return sql;
  if (/\bWITH\b/i.test(sql)) return sql;
  const hasLimit =
  ...
  const orderByMatch =
    /\bORDER\s+BY\s+([A-Za-z_]\w*(?:\.[A-Za-z_]\w*)?)\b(?!\()(?:\s+(ASC|DESC))?/i.exec(sql);
  ...
  if (isNotNullRe.test(sql)) return sql;
...
function stripTrailingStrayLetter(sql: string, fixes: string[]): string {
