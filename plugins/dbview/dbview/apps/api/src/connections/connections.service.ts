...
export class ConnectionsService implements OnModuleInit {
  ...
  async create(dto: CreateConnectionDto, principal: AuthPrincipal): Promise<ConnectionSummary> {
    ...
    const sharing = this.validateSharing(dto.sharing ?? DEFAULT_SHARING);
  ...
  updateSharing(
    ...
    const sharing = this.validateSharing(dto.sharing);
  ...
  private validateSharing(sharing: ConnectionSharing): ConnectionSharing {
