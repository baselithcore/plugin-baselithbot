...
export function EmptyState({ onPick }: Props) {
  ...
  const description =
    ui?.empty_state || 'Domande ancorate alle fonti caricate. Ogni risposta cita verbatim.';
  ...
  const refreshPending = useCallback(async () => {
    try {
      ...
      setRunning(jobsR.jobs.filter((j) => j.status === 'queued' || j.status === 'running').length);
    } catch {
      setPending(null);
    }
  }, []);
            ...
            >
              ...
              >
                <ul className="flex flex-col gap-1 text-[11.5px]">
                  ...
                  <li>
                    Scegli una domanda qui sotto, oppure scrivi liberamente. Le risposte citano
                    sempre le fonti.
                  </li>
                  <li>
                    Il contesto viene espanso automaticamente seguendo le relazioni del knowledge
                    graph — utile per domande comparative o trasversali.
                  </li>
                </ul>
              </Hint>
            </motion.div>
