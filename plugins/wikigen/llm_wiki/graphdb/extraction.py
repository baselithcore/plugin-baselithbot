...
def extract_from_page(
    *,
    ...
    try:
        ...
        logger.warning("[graph.extract] LLM call failed for %s: %s", page_id, exc)
        return ExtractionPayload()

    validated = _validate_against_spec(raw, pack.graph)
    _persist(validated, page_id=page_id, page_type=page_type, store=store)
    logger.info(
