...
messages from per-locale JSON catalogs under ``locales/``. English (``en``) is
the default and fallback; Italian (``it``) ships complete alongside it. Missing
keys degrade to the key id rather than raising, so a forgotten translation never
500s a response.
"""
