...
def ingest_raw_file(
    ...
    *,
    ...
    # Graph extraction is deferred to a dedicated phase after generation:
    # its per-page LLM call is independent of generation, so on the OpenAI
    # parallel path we fan those calls out instead of paying them serially
    # inside the as-completed loop. Persists serially — no MERGE races.
    graph_pending: list[Path] = []
    ...
    def _post_write(entry: PagePlan, page_result: PageResult) -> None:
        ...
        if not dry_run and page_result.status in {"written", "needs-review"}:
            graph_pending.append(page_result.target_path)
    ...
    # Knowledge-graph extraction phase (best-effort). Parallel LLM calls on
    # the OpenAI path, serial persist; no-op when GRAPH_EXTRACT_ENABLED=false.
    _run_graph_extraction(
        graph_pending, parallel=parallel_derived, workers=INGEST_MAX_CONCURRENT
    )
