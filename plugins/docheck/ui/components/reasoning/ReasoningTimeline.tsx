...
function Step({ step, idx, last }: { step: ReasoningStep; idx: number; last: boolean }) {
  const [open, setOpen] = useState(false);
  const hasIO = step.input || step.output;

  return (
    <li className="relative animate-slide-up" style={{ animationDelay: `${idx * 40}ms` }}>
      ...
      <div className="rounded-lg border border-border bg-bg-canvas p-3">
        ...
        {step.thought && (
          <p className="mt-2 text-[12px] leading-5 italic text-text-secondary">{step.thought}</p>
        )}
