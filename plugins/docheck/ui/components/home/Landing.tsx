...
function Hero() {
  ...
  const engineReady = !health.isError && !!health.data && health.data.status === 'ok';
  return (
      ...
      <div className="flex flex-col justify-between rounded-lg border border-border bg-bg-panel p-6 shadow-panel">
        <div>
          <div className="inline-flex items-center gap-2 rounded-md border border-border bg-bg-panel-elev px-2.5 py-1.5 text-xs font-medium text-text-secondary">
            <ShieldCheck
              size={13}
              className={engineReady ? 'text-status-success' : 'text-status-warning'}
            />
            {engineReady
          ...
          <p className="mt-4 max-w-2xl text-sm leading-6 text-text-muted">{t('subtitle')}</p>
        </div>
          ...
          <CardContent className="pt-5 pb-5">
            <div className="mb-6 flex items-center justify-between">
              <div className="text-left">
                ...
                <h2 className="text-base font-semibold text-text-primary">{t('uploadHeading')}</h2>
              </div>
              <div
                className={cn(
              ...
              >
                <StatusDot tone={engineReady ? 'success' : 'warning'} pulse={engineReady} />
                {engineReady
                  ? t('engineReady', { version: health.data?.version ?? '' })
                  : health.isLoading
                    ? t('engineProbing')
...
function SignalsRow() {
  ...
  const ratioPct = typeof verdictRatio === 'number' ? `${Math.round(verdictRatio * 100)}%` : '—';
  const total = queue.data
    ? queue.data.in_review.value + queue.data.pending_approval.value + queue.data.compliant.value
    : null;
  return (
    <div className="mt-8 grid grid-cols-1 gap-3 sm:grid-cols-3">
      <Signal icon={Network} label={t('fanout')} value={t('fanoutValue')} />
      <Signal icon={LockKeyhole} label={t('cache')} value={cache.isLoading ? '…' : ratioPct} />
      <Signal
        icon={GaugeCircle}
        label={t('tracked')}
...
function Signal({ icon: Icon, label, value }: { icon: LucideIcon; label: string; value: string }) {
  return (
    <div className="rounded-lg border border-border bg-bg-canvas/70 p-3 transition-colors hover:border-border-strong">
      ...
      <div className="mt-3 text-[10px] uppercase tracking-wide text-text-muted">{label}</div>
  ...
  return (
    <Card interactive className="relative overflow-hidden p-5 group">
      <div className="relative z-10 flex items-center justify-between">
        <span className="font-mono text-xs uppercase tracking-widest text-text-muted">{step}</span>
      ...
      <p className="relative z-10 mt-3 text-sm leading-relaxed text-text-secondary">{text}</p>
    </Card>
  );
}
