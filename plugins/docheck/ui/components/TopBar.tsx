      ...
      <header className="h-14 border-b border-border bg-bg-panel flex items-center justify-between px-4 lg:px-6 sticky top-0 z-30">
        ...
        <div className="flex items-center gap-2 lg:gap-2.5">
          ...
          <Tooltip>
            ...
            <TooltipContent side="bottom">{t('policiesTooltip')}</TooltipContent>
          </Tooltip>
          ...
          <div className="relative" ref={menuRef}>
            ...
            >
              ...
              <ChevronDown
                size={13}
                className={cn('text-text-muted transition-transform', menu && 'rotate-180')}
              />
            </button>
            {menu && (
              <div className="absolute right-0 top-10 w-72 surface-elev border border-border rounded-lg shadow-popover p-2 z-40 animate-slide-up">
                <div className="px-3 py-2.5 border-b border-border/50">
                  ...
                  <div className="mt-2.5 flex items-center justify-between text-[10px] text-text-muted">
                    ...
                    <span className="font-mono text-text-secondary">{tenant}</span>
                  </div>
                </div>
                <button
                  type="button"
    ...
    <Tooltip>
      ...
      <TooltipContent side="bottom">
        <div className="text-[11px]">
          <div className="font-medium text-text-primary">{t('modelTooltipTitle')}</div>
          <div className="mt-0.5 text-text-muted">
            {health.data?.llm_model ?? '—'} · {health.data?.llm_base_url ?? ''}
          </div>
          <div className="mt-0.5 text-text-muted">{t('modelTooltipEgress')}</div>
        </div>
      </TooltipContent>
    </Tooltip>
  );
