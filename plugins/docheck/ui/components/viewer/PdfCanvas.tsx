...
export function PdfCanvas({ url, selected, chunks }: Props) {
  ...
  if (error) {
    return <div className="p-6 text-sm text-status-danger">PDF error: {error}</div>;
  }

  return (
    <div ref={containerRef} className="space-y-4 p-4 bg-bg-canvas">
      {pageCount === 0 && <p className="text-xs text-text-muted">Loading PDF…</p>}
      {chunks.map((c) => {
...
function bboxStyle(bbox: string): React.CSSProperties {
  try {
    const [x0, y0, x1, y1] = JSON.parse(bbox) as [number, number, number, number];
