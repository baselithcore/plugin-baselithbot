...
export function AuditDetailDrawer({ seq, onClose }: { seq: number; onClose: () => void }) {
      ...
      >
        ...
        {detail.data && (
          <div className="space-y-3 text-xs">
            ...
            <Row label={t('user')}>{detail.data.user_email ?? detail.data.user_id ?? '—'}</Row>
            <Row label={t('action')}>
              <span className="font-mono">{detail.data.action}</span>
            </Row>
            <Row label={t('resource')}>
              <span className="font-mono break-all">{detail.data.resource ?? '—'}</span>
            </Row>
            <Hash
              label={t('payloadHash')}
