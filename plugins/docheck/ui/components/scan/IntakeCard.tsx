...
import { CheckCircle2, FileText, Layers, Upload, type LucideIcon } from 'lucide-react';
...
export function IntakeCard({ file, onFile, pickedDocId, onPick, docs, docsLoading }: Props) {
  const t = useTranslations('scan.intake');
  const [tab, setTab] = useState<'upload' | 'vault'>('upload');
  return (
    <Card>
      <CardContent className="pt-5">
        <StepHeader idx={1} icon={FileText} title={t('title')} hint={t('hint')} />
        ...
        {tab === 'upload' ? (
          ...
          <VaultList docs={docs} loading={docsLoading} pickedDocId={pickedDocId} onPick={onPick} />
        )}
      </CardContent>
    </Card>
  );
...
function UploadBox({ file, onFile }: { file: File | null; onFile: (f: File | null) => void }) {
  const t = useTranslations('scan.intake');
  return (
    <label
      className={cn(
    ...
    <div className="rounded-lg border border-border bg-bg-canvas max-h-64 overflow-auto divide-y divide-border">
      {docs.map((d) => (
        ...
        >
          <FileText
            size={14}
            className={pickedDocId === d.id ? 'text-status-info' : 'text-text-muted'}
          />
          <div className="min-w-0 flex-1">
            <div className="truncate text-xs font-medium">{d.filename}</div>
            <div className="font-mono text-[10px] text-text-muted truncate">
              {d.id} · {d.pages ?? '?'}p{d.doc_type ? ` · ${t(`docType.${d.doc_type}`)}` : ''}
            </div>
          </div>
          {pickedDocId === d.id && <CheckCircle2 size={13} className="text-status-info shrink-0" />}
        </button>
      ))}
    </div>
  );
