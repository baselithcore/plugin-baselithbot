...
export function VerdictCard({
      ...
      <Card>
        <CardContent className="pt-5">
          <div className="flex flex-col items-center justify-center text-center py-8">
            ...
            <p className="mt-1 text-xs text-text-muted max-w-sm">{t('noScanText')}</p>
          </div>
        </CardContent>
      </Card>
    );
        ...
        <CardContent className="pt-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[11px] text-text-muted">{t('willAppear')}</div>
            </div>
          </div>
        </CardContent>
      </Card>
  ...
  const tone = verdict === 'compliant' ? 'success' : verdict === 'critical' ? 'danger' : 'warning';
  const verdictCls = {
        ...
        <div className="flex items-start gap-5">
          ...
          <div className="min-w-0 flex-1">
            <div className="flex items-center justify-between gap-3">
              ...
              <Button size="sm" variant="ghost" onClick={onRetrySummary} disabled={summaryLoading}>
            ...
            <h3 className="mt-3 text-base font-semibold tracking-tight">
              {summaryLoading
                ? t('judging')
                : summary?.headline ||
                  (summaryError ? t('headlineUnavailable') : t('headlineAwaiting'))}
            </h3>
            <p className="mt-2 text-sm leading-6 text-text-secondary">
              {summaryLoading ? (
                ...
                <span className="text-status-warning text-xs">{summaryError}</span>
              ) : null}
            </p>
          </div>
        </div>
