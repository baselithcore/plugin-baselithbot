...
export function PolicyCard({ policies, loading, selected, onChange }: Props) {
  ...
  const allActive = policies.filter((p) => p.active && !p.system).map((p) => p.id);
      ...
      <CardContent className="pt-5">
        ...
        <div className="rounded-lg border border-border bg-bg-canvas max-h-56 overflow-auto divide-y divide-border">
          {loading ? (
            ...
            <div className="p-4 text-center text-xs text-text-muted">{t('noMatches')}</div>
          ) : (
            list.map((p) => (
              <PolicyRowItem
                key={`${p.id}@${p.version}`}
...
function PolicyRowItem({
  ...
  return (
    <button
      ...
      onClick={() => toggle(p.id)}
      disabled={sys}
      title={sys ? 'Policy di sistema — sempre attiva, non modificabile' : undefined}
      className={cn(
