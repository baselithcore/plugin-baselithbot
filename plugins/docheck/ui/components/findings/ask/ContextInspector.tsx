...
import { ChevronDown, FileSearch, Quote, ScrollText, ShieldCheck } from 'lucide-react';
    ...
    <div className="border-b border-border bg-bg-panel-soft/40">
      ...
      >
        <span className="inline-flex items-center gap-2">
          ...
          {t('contextLabel')}
          <span className="text-[10px] text-text-muted font-mono">({t('contextHint')})</span>
        </span>
        <ChevronDown
          size={14}
          ...
          {finding.reasoning?.length > 0 && (
            <Block
              ...
              body={
                finding.reasoning
                  .slice(0, 3)
                  .map((s) => `• ${s.agent}${s.thought ? ` — ${s.thought}` : ''}`)
  ...
  return (
    <div className="rounded-md border border-border bg-bg-canvas px-3 py-2">
      <div className="flex items-center justify-between text-[10px] uppercase tracking-wide text-text-muted">
        ...
        <span className="font-mono normal-case truncate ml-2 max-w-[60%]">{meta}</span>
      </div>
