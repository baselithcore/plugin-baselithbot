...
export function AgentPipeline({ phase, progress, done, error, variant = 'compact' }: Props) {
  ...
  if (variant === 'full') {
    return (
      <div className="rounded-xl border border-border surface-elev p-4 shadow-panel animate-fade-in">
        <Header done={done} progress={progress} overall={overall} phase={phase} />
  ...
  return (
    <div className="flex flex-col gap-2 animate-fade-in">
      <div className="flex items-center gap-3 rounded-md border border-border bg-bg-panel-elev px-3 py-2">
        ...
        <div className="flex flex-1 items-center gap-1" aria-label={t('ariaStages')}>
          {stages.map((stage, idx) => {
            const st = statusFor(idx);
            return (
              <span key={stage.id} className="group flex flex-1 items-center" title={stage.label}>
                <span
                  className={cn(
                    'h-1.5 flex-1 rounded-full transition-all duration-300',
                    st === 'done' && 'bg-status-success',
                    st === 'active' && 'bg-status-info shadow-[0_0_12px_rgba(47,123,255,0.6)]',
  ...
  return (
    <div className="flex items-center justify-between gap-4">
      ...
      <div className="text-right">
        <div className="font-mono text-[11px] uppercase tracking-wide text-text-muted">Overall</div>
        <div className="text-2xl font-semibold tabular-nums text-text-primary">{overall}%</div>
      </div>
    </div>
  );
}
    ...
    >
      <div
        className={cn(
          ...
          status === 'done' && 'border-status-success/30 bg-status-success/10 text-status-success',
          status === 'active' && 'border-status-info/40 bg-status-info/15 text-status-info',
      ...
      <div className="mt-2.5 text-[11px] font-semibold text-text-primary">{stage.label}</div>
      <div className="mt-0.5 text-[10px] leading-4 text-text-muted">{stage.desc}</div>
      {status === 'active' && (
