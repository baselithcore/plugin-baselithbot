...
import { DEFAULT_PROFILE, FLOOR_RANK, type ScanProfile } from '@/components/scan/types';
...
export default function ScanPage() {
  ...
  useEffect(() => {
    if (policyIds.length === 0 && policies.data) {
      const active = policies.data.filter((p) => p.active && !p.system).map((p) => p.id);
      ...
      <main className="flex-1 overflow-auto bg-bg-canvas">
        <div className="mx-auto max-w-7xl px-6 lg:px-8 py-7">
          ...
          <section className="mt-6 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1.4fr)]">
            ...
            />
            <ResultsCard report={report} findings={filtered} profile={profile} phase={phase} />
          </section>
        </div>
      </main>
      <FindingDetailDrawer />
  ...
  return (
    <div className="flex items-center justify-between gap-6">
      <div>
        ...
        <p className="mt-1.5 text-sm text-text-muted max-w-2xl">{t('description')}</p>
      </div>
