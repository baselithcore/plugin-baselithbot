...
def _pred(
    rule_id: str, severity: str, text: str, agent: str = "LegalComplianceAgent"
) -> dict:
    return {
        "id": f"pred-{rule_id}",
        "rule_id": rule_id,
...
def test_score_case_perfect_match() -> None:
    case = _case(
        ...
        [
            ExpectedFinding(
                rule_id="R1", severity="FAIL", evidence_contains="trattamento"
            )
        ],
    )
    score = score_case(
        case, [_pred("R1", "FAIL", "I dati saranno trattamento conforme")]
    )
    assert score.tp == 1
    assert score.fp == 0
    assert score.fn == 0
...
def test_score_case_each_expected_matched_once() -> None:
    ...
    score = score_case(
        case, [_pred("R1", "FAIL", "x here"), _pred("R1", "FAIL", "x here too")]
    )
    assert score.tp == 1
    assert score.fp == 1
    assert score.fn == 0
...
def test_aggregate_per_rule_and_per_agent() -> None:
    ...
    assert {a.agent for a in result.per_agent} == {
...
def test_baseline_roundtrip(tmp_path: Path) -> None:
    base = Baseline(
        version="2026-05-14",
        metrics={"precision": 0.8, "recall": 0.7, "f1": 0.747},
        tolerance=0.02,
    )
...
def test_check_gate_pass_within_tolerance() -> None:
    case = _case(
        "c1", [ExpectedFinding(rule_id="R1", severity="FAIL", evidence_contains="x")]
    )
