...
def _structurer_response() -> dict:
    return {"structure": [
        {"id": "s1", "type": "title", "label": "Documento", "parent_id": None,
         "page": 1, "line_start": 1, "line_end": 1, "chunk_ids": []},
    ]}
...
def _legal_response(messages: list[ChatMessage]) -> dict:
    ...
    return {"findings": [{
        ...
        "confidence": 0.92,
        "reasoning": [
            {"action": "retrieve_policy", "input": {"query": "retention"}, "output": {"rule_ids": ["GDPR-Art-13"]}},
...
def _synth_response() -> dict:
    return {"score": 78, "summary": "[MOCK] Analisi mock.",
            "by_severity": {"FAIL": 1, "WARN": 0, "PASS": 0}, "top_risks": []}
...
async def chat_completions(req: ChatRequest):
    ...
    payload: dict
    match agent:
        case "structurer":  payload = _structurer_response()
        case "legal":       payload = _legal_response(req.messages)
        case "pii":         payload = _pii_response()
        case "synthesizer": payload = _synth_response()
        case _:             payload = {}
