...
async def main() -> None:
    ...
    coll.upsert(
        ids=rule_ids, documents=excerpts, metadatas=metadatas, embeddings=vectors
    )
    print(
        f"Seeded {len(rule_ids)} rules across {len(SEED_POLICIES)} policies for tenant '{args.tenant}'."
    )
