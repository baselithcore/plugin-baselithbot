 ...
 */
export function serializeUsage(rows) {
    return rows
        .filter((row) => row.promptTokens > 0 || row.completionTokens > 0)
        .map((row) => `${sanitizeModel(row.model)};${Math.trunc(row.promptTokens)};${Math.trunc(row.completionTokens)}`)
        .join(',');
}
...
//# sourceMappingURL=llm-usage.store.js.map