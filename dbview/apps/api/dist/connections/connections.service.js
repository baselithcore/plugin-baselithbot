...
import { DIALECT_META, DbviewError, ForbiddenError, buildConnectionString, parseSalesforceConnection, parseSalesforceDataCloudConnection, } from '@dbview/shared';
 ...
 */
const DEFAULT_SHARING = { mode: 'private', userIds: [] };
let ConnectionsService = class ConnectionsService {
    auth;
    enginePool;
    logger = new Logger('ConnectionsService');
    store = new ConnectionsStore();
    constructor(auth, enginePool) {
        this.auth = auth;
    ...
    updateSharing(id, dto, principal) {
        if (principal.role !== 'admin') {
    ...
    uploadDump(req) {
        ...
        const id = randomUUID();
        const baseName = req.format === 'sqlite-sql' ? `${id}.db` : `${id}-${sanitizeFilename(req.filename)}`;
        ...
        try {
            ...
            throw new DbviewError(`Dump import failed: ${err.message}`, 'dump_import_failed', 400);
        }
    }
    resolvePlain(id, principal) {
        const c = this.requireById(id);
        if (!this.canView(c, principal)) {
            throw new NotFoundException(`Connection ${id} not found.`);
        }
    ...
    canView(c, principal) {
        ...
        if (!this.auth.sameTenant(c.ownerId, principal))
            return false;
        if (c.sharing.mode === 'all')
            return true;
        if (c.sharing.mode === 'users' && c.sharing.userIds.includes(principal.id))
            return true;
        if (c.sharing.mode === 'admins' && principal.role === 'admin')
            return true;
        return false;
    }
    /**
     ...
     */
    logCrossAdminMutation(c, principal, action) {
        if (principal.role !== 'admin' || c.ownerId === principal.id)
            return;
        this.logger.warn(`connection_cross_admin_${action} id=${c.id} owner=${c.ownerId} actor=${principal.id}`);
    }
    requireById(id) {
        const c = this.store.get(id);
        if (!c)
    ...
    validateSharing(sharing, principal) {
        ...
        for (const uid of unique) {
            ...
            if (!user || !user.isActive || !this.auth.sameTenant(uid, principal)) {
                throw new DbviewError(`sharing.userIds references unknown or inactive user: ${uid}`, 'invalid_sharing_target', 400);
            }
        }
    ...
    backfillLegacyOwnership() {
        ...
        if (legacyPending.length === 0 && privatePending.length === 0)
            return;
        const admin = this.auth.listUsers().find((u) => u.role === 'admin' && u.isActive);
        if (!admin) {
            this.logger.warn(`legacy_connections_pending count=${legacyPending.length + privatePending.length} no_admin_yet — will retry on first access`);
            return;
        }
        ...
        if (legacyPending.length > 0) {
            this.logger.log(`legacy_connections_migrated count=${legacyPending.length} owner=${admin.id}`);
        }
        if (privatePending.length > 0) {
            this.logger.log(`v1_private_upgraded_to_admins count=${privatePending.length} (preserves cross-admin visibility)`);
        }
    }
    materialize(dto) {
        const meta = DIALECT_META[dto.dialect];
        if (!meta.available) {
            throw new DbviewError(`Dialect '${dto.dialect}' is not yet available. Coming soon.`, 'dialect_unavailable', 400);
        }
        if (dto.params)
            return buildConnectionString(dto.params);
        if (dto.connectionString)
            return dto.connectionString;
        throw new DbviewError('Either params or connectionString is required.', 'invalid_connection_payload', 400);
    }
    async assertReachable(dialect, connectionString) {
        ...
        try {
            ...
            throw new DbviewError(`Connection test failed: ${err.message}`, 'connection_test_failed', 400);
        }
        finally {
            await engine.close();
        }
...
function parseDisplay(cs, dialect) {
    if (dialect === 'sqlite' || dialect === 'duckdb')
        return { database: cs.split('/').pop() };
    ...
    try {
        const normalized = dialect === 'falkordb'
