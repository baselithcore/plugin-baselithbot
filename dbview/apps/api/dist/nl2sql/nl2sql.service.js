...
import { LlmProviderError, ModelOutputError, SchemaMismatchError, UnsafeSqlError, dialectKind, isGraphDialect, isSaasDialect, isSqlDialect, } from '@dbview/shared';
...
import { buildSystemPrompt, buildUserPromptBase, renderHistoryBlock, withRetryFeedback, } from './prompt.js';
...
function clampRetries(raw, fallback) {
    if (!raw)
        return fallback;
...
function clampNumber(raw, fallback, lo, hi) {
    if (!raw)
        return fallback;
    const n = Number.parseInt(raw, 10);
    if (!Number.isFinite(n))
...
let Nl2SqlService = class Nl2SqlService {
    connections;
    schema;
    governance;
    logger = new Logger('Nl2SqlService');
    ...
    constructor(connections, schema, governance) {
        this.connections = connections;
        this.schema = schema;
        this.governance = governance;
    }
    async translate(req, principal) {
        ...
        const { provider: llmProvider, model, auth, } = this.governance.resolveTranslate(req.provider, req.model, kind, principal);
        ...
        if (fullGraph.kind === 'relational') {
            ...
            if (pruneResult.pruned) {
                ...
                usingPruned = true;
                this.logger.log(`nl2query schema-pruned ${fullGraph.tables.length}->${pruneResult.graph.tables.length} tokens=[${pruneResult.matchedTokens.slice(0, 8).join(',')}]`);
            }
        }
        ...
        for (let attempt = 0; attempt <= MAX_RETRIES; attempt++) {
            ...
            try {
                ...
                if (err instanceof ModelOutputError) {
                    ...
                    lastError = err;
                    this.logger.warn(`nl2query model-output-invalid attempt=${attempt} adapter=${adapter.name ?? adapter.provider} model=${model}: ${err.message}`);
                    continue;
                }
                throw err;
            }
            ...
            try {
                ...
                feedback = `Output was not valid JSON: ${err.message}`;
                lastError = err;
                this.logger.warn(`nl2query parse-error attempt=${attempt} adapter=${adapter.name ?? adapter.provider} model=${model} dialect=${conn.dialect}: ${err.message}`);
                continue;
            }
            try {
                if (parsed.language === 'sql' || parsed.language === 'cypher') {
                    const sanitized = sanitize(parsed.query, {
                        ...
                        knownTables: graph.kind === 'relational' ? new Set(graph.tables.map((t) => t.id)) : undefined,
                        stripAllSchemaPrefixes: conn.dialect === 'sqlite' || conn.dialect === 'salesforce-data-cloud',
                    });
                    if (sanitized.fixes.length > 0) {
                        this.logger.warn(`nl2query sanitized attempt=${attempt} dialect=${conn.dialect} fixes=[${sanitized.fixes.join('; ')}]`);
                ...
                this.logger.log(`nl2query candidate attempt=${attempt} dialect=${conn.dialect} query=${JSON.stringify(parsed.query)}`);
                const annotated = this.validate(parsed, graph, req);
                const explanationMissing = !annotated.explanation || annotated.explanation.trim().length === 0;
                const joinNotesMissing = annotated.joinNotes.length === 0;
                if ((explanationMissing || joinNotesMissing) &&
                    (annotated.language === 'sql' || annotated.language === 'cypher')) {
                    const explainGov = this.governance.resolveExplain(req.provider, principal);
                    const explainer = instrumentLlmAdapter(createChatAdapter(explainGov.provider, explainGov.auth), 'explain');
                    ...
                    this.logger.log(`nl2query explained adapter=${explainer.name ?? explainer.provider} model=${explainModel} durationMs=${Date.now() - explainStart} hasExplanation=${enriched.explanation.length > 0}`);
                }
                const durationMs = Date.now() - t0;
                this.logger.log(`nl2query ok adapter=${adapter.name ?? adapter.provider} model=${model} dialect=${conn.dialect} kind=${kind} retries=${attempt} durationMs=${durationMs} entities=${annotated.involvedEntities.length}`);
                return {
                    ...annotated,
                    dialect: conn.dialect,
                ...
                if (err instanceof UnsafeSqlError) {
                    ...
                    if (usingPruned && unknown.length > 0 && fullGraph.kind === 'relational') {
                        ...
                        if (recoverable) {
                            ...
                            this.logger.log(`nl2query schema-pruned-fallback attempt=${attempt} unknown=[${unknown.join(',')}] reverted to full schema`);
                        }
                    }
                    if (unknown.length > 0) {
                        const suggestions = buildSuggestionMap(unknown, knownEntities);
                        const hint = formatSuggestions(suggestions, knownEntities);
                        const columnLocations = graph.kind === 'relational' ? buildColumnLocationHint(unknown, graph) : '';
                    ...
                    this.logger.warn(`nl2query unsafe attempt=${attempt} adapter=${adapter.name ?? adapter.provider} model=${model} dialect=${conn.dialect} unknown=[${unknown.join(',')}]: ${err.message}`);
        ...
        this.logger.error(`nl2query failed adapter=${adapter.name ?? adapter.provider} model=${model} dialect=${conn.dialect} retries=${attempts} lastQuery=${JSON.stringify(lastRejectedQuery ?? '')}: ${lastError?.message ?? 'unknown'}`);
        if (groundingFailure) {
            const suggestions = buildSuggestionMap(groundingFailure.unknown, knownEntities);
            const columnLocations = graph.kind === 'relational'
                ? buildColumnLocationMap(groundingFailure.unknown, graph)
                : undefined;
            throw new SchemaMismatchError(`Could not generate a query grounded in the schema after ${attempts} attempts. ${groundingFailure.reason}`, {
                attempts,
        ...
        const tail = lastRejectedQuery
            ...
            : '';
        throw new LlmProviderError(`NL2Query failed after ${attempts} attempts: ${lastError?.message ?? 'unknown'}.${tail}`);
    }
    validate(parsed, graph, req) {
        ...
        if (graph.kind === 'vector') {
            ...
            let envelope;
            try {
                envelope = JSON.parse(parsed.query);
            }
            catch (err) {
                throw new UnsafeSqlError(`Qdrant query must be a JSON envelope string: ${err.message}`);
            }
            const opName = typeof envelope.op === 'string' ? envelope.op : '';
            if (!opName)
                throw new UnsafeSqlError('Qdrant envelope missing "op".');
            ...
            if (opName !== 'collections') {
                ...
                if (!collection)
                    throw new UnsafeSqlError(`Qdrant op '${opName}' missing "collection".`);
 ...
 */
function buildColumnLocationMap(unknown, graph) {
    const out = {};
...
function buildColumnLocationHint(unknown, graph) {
    const map = buildColumnLocationMap(unknown, graph);
    const lines = Object.entries(map).map(([col, owners]) => `Column '${col}' exists on: ${owners.join(', ')}.`);
    return lines.length > 0 ? ` ${lines.join(' ')} Pick the correct table/alias.` : '';
}
function collectKnownEntities(graph) {
...
function extractUnknownEntities(message) {
    const out = new Set();
    for (const re of [
        UNKNOWN_TABLE_RE,
        UNKNOWN_COLUMN_RE,
...
function formatSuggestions(suggestions, available) {
    const parts = [];
    const entries = Object.entries(suggestions);
    if (entries.length > 0) {
        const lines = entries.map(([k, v]) => `'${k}' → did you mean ${v.map((x) => `'${x}'`).join(', ')}?`);
...
function parseJsonResponse(text) {
    ...
    const lang = obj.language === 'cypher' ? 'cypher' : obj.language === 'qdrant' ? 'qdrant' : 'sql';
    return {
        query,
        language: lang,
...
//# sourceMappingURL=nl2sql.service.js.map