 ...
 */
export async function summarizeResult(adapter, model, input) {
 ...
 */
export function renderSchemaDigest(schema) {
    if (schema.kind === 'relational') {
        const lines = ['Tables (table → columns):'];
    ...
    if (schema.kind === 'graph') {
        const lines = ['Node labels (label → properties):'];
    ...
    if (schema.kind === 'vector') {
        const lines = ['Collections (collection → payload fields):'];
    ...
    if (schema.kind === 'keyvalue') {
        const lines = ['Namespaces (pattern → types, key count):'];
    ...
    if (schema.kind === 'search') {
        const lines = ['Indices (name → fields):'];
    ...
    // document
    const lines = ['Collections (name → fields):'];
    for (const c of schema.collections) {
...
function collectSchemaVocabulary(schema) {
    const ids = new Set();
    const add = (raw) => {
        if (!raw)
            return;
 ...
 */
export function isGroundedInSchema(question, vocab) {
 ...
 */
export function detectAllNullColumns(result) {
...
export function parseSummaryJson(text) {
    ...
    try {
        ...
        const summary = typeof obj.summary === 'string' && obj.summary.trim().length > 0 ? obj.summary.trim() : '';
        if (!summary)
            return null;
        const highlights = pickStringArray(obj.highlights, MAX_HIGHLIGHTS);
        const followUpsRaw = obj.followUps ?? obj.follow_ups;
        const followUps = sanitizeFollowUps(pickStringArray(followUpsRaw, MAX_FOLLOW_UPS * 2)).slice(0, MAX_FOLLOW_UPS);
...
function pickStringArray(input, max) {
 ...
 * is the safety net.
 */
export function sanitizeFollowUps(items) {
    ...
    for (const raw of items) {
        ...
        if (q.length < 8)
            continue;
        if (ANAPHORA_PATTERNS.some((re) => re.test(q)))
            continue;
...
function deterministicFallback(result, locale) {
    if (locale === 'it') {
...
function formatRowInline(columns, row) {
    const parts = [];
