import { LlmProviderError, ModelOutputError, } from '@dbview/shared';
/**
...
export class SqlcoderAdapter {
    ...
    async complete(input, model) {
        if (!input.structured) {
            throw new LlmProviderError(`sqlcoder adapter requires structured context (schema, dialect, userPrompt).`);
        }
        if (input.structured.schema.kind !== 'relational') {
            throw new LlmProviderError(`sqlcoder only supports relational schemas; got '${input.structured.schema.kind}'.`);
        }
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new LlmProviderError(`sqlcoder (${model}) returned ${res.status}: ${text.slice(0, 200) || res.statusText}`);
            }
            const json = (await res.json());
            if (json.error)
                throw new LlmProviderError(`sqlcoder (${model}) error: ${json.error}`);
            const raw = json.response ?? '';
            if (!raw.trim())
                throw new LlmProviderError(`sqlcoder (${model}) returned empty content.`);
            const sql = extractSql(raw);
            if (!sql) {
                throw new ModelOutputError(`sqlcoder (${model}) output did not contain a SELECT/WITH statement; got: ${raw.slice(0, 200).replace(/\n/g, ' ')}`);
            }
            ...
            throw new LlmProviderError(`sqlcoder (${model}) failed: ${err.message}`);
        }
        finally {
 ...
 */
export function renderSqlcoderPrompt(ctx) {
    ...
    const namingRule = ctx.dialect === 'sqlite'
...
function pickTableExample(schema) {
 ...
 */
function renderPriorTurns(history) {
    ...
    lines.push('-- Treat the new question as a follow-up; resolve references ("these", "previous", "now", "il precedente") against these turns. Re-emit a full new SQL — do not copy verbatim.');
    lines.push('');
    return lines.join('\n');
}
...
//# sourceMappingURL=sqlcoder-adapter.js.map