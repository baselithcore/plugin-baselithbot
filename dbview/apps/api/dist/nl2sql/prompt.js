...
const SQL_DIALECT_NOTES = {
    postgres: 'Use PostgreSQL syntax. Quoted identifiers use double quotes. Use ILIKE for case-insensitive matches.',
    mysql: 'Use MySQL 8 syntax. Quoted identifiers use backticks. Use LIKE (case-insensitive by default with utf8mb4_*_ci collations).',
    mariadb: 'Use MariaDB 10+ syntax. Quoted identifiers use backticks. Use LIKE for matches. Avoid features that require MySQL 8 only.',
    mssql: 'Use T-SQL (SQL Server) syntax. Quoted identifiers use square brackets [name]. Limit results with `SELECT TOP n` or `OFFSET 0 ROWS FETCH NEXT n ROWS ONLY` (do NOT use LIMIT — it is invalid in T-SQL). Use COLLATE for case-insensitive matches when needed.',
    sqlite: 'Use SQLite syntax. No RIGHT/FULL OUTER JOIN. No window functions older than 3.25. Use LIKE for matches.',
    cockroach: 'Use CockroachDB SQL (PostgreSQL-compatible). Quoted identifiers use double quotes. Use ILIKE for case-insensitive matches. Prefer indexed columns in WHERE clauses; avoid full table scans on large tables.',
    oracle: "Use Oracle SQL (PL/SQL dialect). Quoted identifiers use double quotes. Use UPPER()/LOWER() for case-insensitive matches. Limit results with `FETCH FIRST n ROWS ONLY` (do NOT use LIMIT). Date literals use TO_DATE('YYYY-MM-DD','YYYY-MM-DD').",
    clickhouse: 'Use ClickHouse SQL. Quoted identifiers use backticks. Use ILIKE for case-insensitive matches. Prefer PREWHERE for selective filters, ORDER BY/LIMIT, and SAMPLE for stats. Use FORMAT clause sparingly. Aggregations are first-class — use uniq()/uniqExact()/quantile() instead of COUNT(DISTINCT) when possible.',
    duckdb: 'Use DuckDB SQL (PostgreSQL-compatible, OLAP-focused). Quoted identifiers use double quotes. Use ILIKE for case-insensitive matches. Window functions, list/struct/map types, and QUALIFY are available. Prefer columnar-friendly aggregations and avoid SELECT *. Use `read_csv_auto`/`read_parquet` for ad-hoc file reads only when filenames are listed in the schema.',
};
export function compactRelationalSchema(graph) {
    ...
    const tableRef = (id, name) => (isSdc ? name : id);
    const lines = [];
...
function stripSdcSchema(id) {
    return id.startsWith('data_cloud.') ? id.slice('data_cloud.'.length) : id;
}
export function compactGraphSchema(graph) {
    const lines = ['Node labels:'];
    for (const l of graph.labels) {
        const props = l.properties
            .map((p) => `  - ${p.name} (type: ${p.types.join('|')}${p.nullable ? '' : ', NOT NULL'})${formatSamples(p.sampleValues)}`)
            .join('\n');
...
export function compactVectorSchema(graph) {
    const lines = [];
...
export function buildSystemPrompt(graph, dialect, allowDml) {
...
function buildSystemPromptCore(graph, dialect, allowDml) {
    ...
    if (graph.kind === 'relational') {
        const dialectNote = dialect in SQL_DIALECT_NOTES
            ? SQL_DIALECT_NOTES[dialect]
    ...
    const engineNote = dialect === 'falkordb'
...
function getSchemaPromptContext(graph) {
    let ctx = schemaContextCache.get(graph);
    if (ctx)
        return ctx;
    const schemaText = graph.kind === 'relational'
...
export function buildUserPromptBase(args) {
...
export function withRetryFeedback(base, retryFeedback) {
    if (!retryFeedback)
        return base.text;
    return [
        base.text,
...
export function renderHistoryBlock(history) {
    ...
    history.forEach((turn, idx) => {
        ...
        if (turn.query) {
            ...
            const rows = turn.rowCount === undefined || turn.rowCount === null ? '' : ` (rows=${turn.rowCount})`;
            const status = turn.ok === false ? ' [failed]' : '';
            lines.push(`Turn ${n} assistant ${lang}${status}${rows}: ${q}`);
        }
    });
    lines.push('');
    lines.push('Treat the current user question as a follow-up to the turns above. Resolve references ("these", "those", "previous", "now", "also", "instead", "il precedente", "questi") against the most recent turns. Re-generate the full query from scratch — do NOT copy the prior query verbatim; apply the modification asked by the new question. If the new question is unrelated, ignore the history.');
    return lines.join('\n');
}
...
function listAllowedEntities(graph) {
    if (graph.kind === 'relational') {
...
export function compactDocumentSchema(graph) {
    const lines = [`MongoDB database: ${graph.database}`];
    for (const c of graph.collections) {
        lines.push(`Collection "${c.name}" — docs: ${c.docCount ?? '?'}${c.indexes.length ? `, indexes: [${c.indexes.join(', ')}]` : ''}`);
        for (const f of c.fields.slice(0, 30)) {
            const samples = f.sampleValues && f.sampleValues.length
                ? ` — values: [${f.sampleValues.map((v) => JSON.stringify(v)).join(', ')}]`
                : '';
            lines.push(`  - ${f.name}: ${f.types.join('|')}${f.presence !== undefined ? ` (${Math.round(f.presence * 100)}%)` : ''}${samples}`);
        }
        if (c.fields.length > 30)
            lines.push(`  … +${c.fields.length - 30} more fields`);
    }
    return lines.join('\n');
}
export function compactSearchSchema(graph) {
    const lines = [];
    if (graph.cluster)
    ...
    for (const idx of graph.indices) {
        ...
        lines.push(`Index "${idx.name}" — docs: ${idx.docCount ?? '?'}, size: ${sizeKB}${idx.aliases.length ? `, aliases: [${idx.aliases.join(', ')}]` : ''}`);
        for (const f of idx.fields.slice(0, 40)) {
            lines.push(`  - ${f.name}: ${f.type}${f.analyzed ? ' (analyzed)' : ''}`);
        }
        if (idx.fields.length > 40)
            lines.push(`  … +${idx.fields.length - 40} more fields`);
    }
    return lines.join('\n');
}
export function compactKeyValueSchema(graph) {
    const lines = [`Redis DB ${graph.db} — ${graph.totalKeys} keys observed`];
