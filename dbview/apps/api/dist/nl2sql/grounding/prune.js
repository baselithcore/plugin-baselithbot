 ...
 */
export function pruneRelationalSchema(graph, prompt, opts) {
    ...
    if (keep.size > opts.maxTables) {
        ...
        for (const extra of closureExtras.slice(0, allowedExtras))
            trimmed.add(extra.id);
        keep.clear();
...
function scoreTable(table, tokens) {
    ...
    for (const tok of tokens) {
        ...
        if (tableName === tok)
            score += 8;
        if (tableNameParts.includes(tok))
            score += 5;
        if (displayParts.includes(tok))
            score += 4;
        if (descParts.includes(tok))
            score += 2;
        // Substring fallback so plurals / partial morphemes still count.
        if (tableName.includes(tok) || tok.includes(tableName))
            score += 2;
        for (const col of table.columns) {
            ...
            if (colName === tok)
                score += 3;
            else if (colParts.includes(tok))
                score += 2;
            if (col.displayName) {
                const dn = col.displayName.toLowerCase();
                if (splitIdentifier(dn).includes(tok))
                    score += 2;
            }
            if (col.description && col.description.toLowerCase().includes(tok))
                score += 1;
        }
    }
    return score;
