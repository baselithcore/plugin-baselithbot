...
const TSQL_EXAMPLES = [
    {
        ...
        question: 'Top 5 orders by amount',
        query: 'SELECT TOP 5 id, customer_id, amount FROM orders WHERE amount IS NOT NULL ORDER BY amount DESC',
    },
    {
        note: 'Pagination — OFFSET ... FETCH NEXT, never LIMIT.',
        question: 'Show me some customers',
        query: 'SELECT id, name, email FROM customers ORDER BY id OFFSET 0 ROWS FETCH NEXT 100 ROWS ONLY',
    },
];
const ORACLE_EXAMPLES = [
    {
        ...
        question: 'Top 5 orders by amount',
        query: 'SELECT id, customer_id, amount FROM orders WHERE amount IS NOT NULL ORDER BY amount DESC FETCH FIRST 5 ROWS ONLY',
    },
];
const SDC_EXAMPLES = [
    {
    ...
    },
    {
        ...
        query: "SELECT ssot__Id__c, ssot__FirstName__c, ssot__LastName__c FROM ssot__Individual__dlm WHERE LOWER(ssot__LastName__c) LIKE LOWER('%rossi%') LIMIT 100",
    },
    {
        ...
        query: "SELECT COUNT(*) AS total FROM Account_Home__dll WHERE LOWER(ECH_FullAddress_c__c) LIKE LOWER('%Rubbiano%')",
    },
];
const SOQL_EXAMPLES = [
    {
...
const CYPHER_EXAMPLES = [
    {
        ...
        query: 'MATCH (c:Customer)-[:PLACED]->(o:Order) RETURN c.name AS name, count(o) AS orders ORDER BY orders DESC LIMIT 5',
    },
    {
        note: 'Text search on title — toLower CONTAINS, never invent label values.',
        question: 'How many pages mention "earthquake"?',
        query: 'MATCH (n:Page) WHERE toLower(n.title) CONTAINS toLower("earthquake") OR toLower(n.text) CONTAINS toLower("earthquake") RETURN count(n) AS total',
    },
];
const QDRANT_EXAMPLES = [
    {
        note: 'Counting → op=count; never op=scroll just to count.',
        question: 'How many documents mention "earthquake"?',
        query: '{"op":"count","collection":"docs","filter":{"must":[{"key":"text","match":{"text":"earthquake"}}]}}',
    },
    {
...
const MONGO_EXAMPLES = [
    ...
    },
    {
        ...
        question: 'Top 5 orders by amount',
        query: '{"op":"find","collection":"orders","filter":{"amount":{"$ne":null}},"projection":{"_id":1,"customer_id":1,"amount":1},"sort":{"amount":-1},"limit":5}',
    },
];
const ES_EXAMPLES = [
    {
 ...
 */
export function pickFewShotExamples(dialect) {
    switch (dialect) {
 ...
 */
export function renderFewShotBlock(examples) {
...
//# sourceMappingURL=few-shot.js.map