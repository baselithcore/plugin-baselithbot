...
const SOQL_DATE_LITERAL_PREFIXES = /^(?:last|next)_n_(?:days|weeks|months|quarters|years|fiscal_quarters|fiscal_years)$/i;
export class SalesforceSafetyValidator {
    validate(soql, opts) {
        const trimmed = soql.trim().replace(/;+\s*$/, '');
        if (!trimmed)
            throw soqlError('Empty SOQL.', 'empty_query');
        if (trimmed.includes(';')) {
        ...
        if (!opts.knownSObjects.has(sobjectName.toLowerCase())) {
            throw soqlError(`Unknown sObject '${sobjectName}'. Use one of the introspected objects.`, 'unknown_table');
        }
        if (opts.knownFields) {
            ...
            if (fieldsSet) {
                ...
                if (unknown.length > 0) {
                    throw soqlError(`Field '${unknown[0]}' not in schema for sObject '${sobjectName}'.`, 'unknown_column');
                }
            }
        }
...
function findUnknownFields(outerQuery, knownFields, sobjectName) {
    ...
    for (const m of cleaned.matchAll(tokenRe)) {
        ...
        if (SOQL_RESERVED.has(lower))
            continue;
        if (SOQL_DATE_LITERAL_PREFIXES.test(tok))
            continue;
        if (lower === sobjectLower)
            continue; // FROM target name itself
        if (seen.has(lower))
            continue;
        seen.add(lower);
...
function soqlError(message, _code) {
