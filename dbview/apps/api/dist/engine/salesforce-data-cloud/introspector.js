...
export class SalesforceDataCloudIntrospector {
    ...
    async introspect() {
        ...
        if (missing.length > 0) {
            ...
            const described = await runWithConcurrency(missing, concurrency, (e) => this.client.describeEntity(e.name));
        ...
        const tables = entities.map((e) => entityToTable(e, fkColumnsByEntity.get(e.name) ?? new Set()));
        return {
            kind: 'relational',
...
function entityToTable(e, fkColumns) {
    const pkSet = new Set();
 ...
 */
function isNoisySystemField(name) {
    return (/^KQ_/i.test(name) ||
 ...
 */
function cleanDisplayName(display, technical) {
    if (!display)
        return undefined;
    ...
    if (norm(trimmed) === norm(technical))
        return undefined;
    return trimmed;
}
function collectEdges(entities, known) {
    const edges = [];
...
function collectFkColumns(edges) {
    const out = new Map();
