...
import { createQueryEngine, type QueryEngine } from '../engine/factory.js';
...
export class QueryService {
  ...
  async sample(req: SampleRequest, principal: AuthPrincipal): Promise<ExecuteQueryResponse> {
    ...
    if (graph.kind === 'relational' && isSqlDialect(dialect)) {
      ...
      if (!table) throw new NotFoundException(`Table '${req.tableId}' not found in schema.`);
    ...
    if (graph.kind === 'relational' && isSaasDialect(dialect)) {
      const table = graph.tables.find((t) => t.id === req.tableId);
      if (!table) throw new NotFoundException(`SObject '${req.tableId}' not found in schema.`);
    ...
    if (graph.kind === 'graph' && isGraphDialect(dialect)) {
      ...
      if (!label) throw new NotFoundException(`Label '${req.tableId}' not found.`);
    ...
    if (graph.kind === 'keyvalue' && isKeyValueDialect(dialect)) {
      ...
      if (!ns) throw new NotFoundException(`Namespace '${req.tableId}' not found.`);
    ...
    if (graph.kind === 'search' && isSearchDialect(dialect)) {
      ...
      if (!idx) throw new NotFoundException(`Index '${req.tableId}' not found.`);
      const envelope = JSON.stringify({
        op: 'search',
        index: idx.name,
    ...
    if (graph.kind === 'document' && isDocumentDialect(dialect)) {
      ...
      if (!coll) throw new NotFoundException(`Collection '${req.tableId}' not found.`);
      const envelope = JSON.stringify({
        op: 'find',
 ...
 */
function stripDataCloudSchemaPrefix(sql: string): string {
