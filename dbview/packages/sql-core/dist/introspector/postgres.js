...
export class PostgresIntrospector {
    pool;
    dialect;
    constructor(connectionString, dialect = 'postgres') {
    ...
    async introspect() {
        ...
        try {
            ...
            throw new IntrospectionError(`${this.dialect === 'cockroach' ? 'CockroachDB' : 'Postgres'} introspection failed: ${formatErr(err)}`);
        }
        finally {
            client?.release();
        }
