    ...
    async introspect() {
        ...
        try {
            ...
            const [colsRes, fksRes] = await Promise.all([
                conn.execute(bindSystemSchemas(COLUMNS_QUERY), [], { outFormat: oracledb.OUT_FORMAT_OBJECT }),
                conn.execute(bindSystemSchemas(FK_QUERY), [], { outFormat: oracledb.OUT_FORMAT_OBJECT }),
            ]);
            await conn.commit();
            ...
            const edges = (fksRes.rows ?? []).map((r, idx) => ({
            ...
            throw new IntrospectionError(`Oracle introspection failed: ${err.message ?? String(err)}`);
        }
        finally {
            await conn?.close();
        }
