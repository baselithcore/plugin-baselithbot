...
function col(name, dataType, flags = {}) {
    return {
        name,
        dataType,
        nullable: true,
...
describe('SqlSafetyValidator — golden patterns', () => {
    ...
    describe('SELECT shapes accepted', () => {
        ...
        it('window function', () => {
            const r = v.validate('SELECT id, total, ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY total DESC) AS rn FROM orders LIMIT 25', opts());
            expect(r.warnings.some((w) => w.code === 'missing_limit')).toBe(false);
        });
        it('chained CTEs do not leak into unknown table errors', () => {
            const sql = 'WITH t1 AS (SELECT id FROM users), t2 AS (SELECT id FROM t1) SELECT id FROM t2 LIMIT 5';
        ...
        it('inner unknown table inside subquery still fails', () => {
            expect(() => v.validate('SELECT id FROM (SELECT id FROM nope) sub LIMIT 5', opts())).toThrow(UnsafeSqlError);
        });
    });
    ...
    describe('separator/multi-statement guard', () => {
        ...
        it('rejects two statements joined by semicolon', () => {
            expect(() => v.validate('SELECT id FROM users LIMIT 1; SELECT id FROM orders LIMIT 1', opts())).toThrow(UnsafeSqlError);
        });
    });
});
//# sourceMappingURL=validator.golden.test.js.map