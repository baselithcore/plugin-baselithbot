...
import { DML_STATEMENT_TYPES, FORBIDDEN_STATEMENT_TYPES, PARSER_DIALECT, } from './rules.js';
export class SqlSafetyValidator {
    parser = new Parser();
    validate(rawSql, opts) {
        const sql = stripComments(rawSql)
            .trim()
            .replace(/;+\s*$/, '');
        if (!sql)
            throw new UnsafeSqlError('Empty SQL after stripping comments.');
        if (sql.includes(';')) {
        ...
        try {
            ...
            throw new UnsafeSqlError(formatParseError(err, sql));
        }
        ...
        if (FORBIDDEN_STATEMENT_TYPES.includes(type)) {
            throw new UnsafeSqlError(`DDL/admin statement '${type.toUpperCase()}' is forbidden.`);
        }
        const warnings = [];
        const isDml = DML_STATEMENT_TYPES.includes(type);
        if (isDml && !opts.allowDml) {
            throw new UnsafeSqlError(`DML statement '${type.toUpperCase()}' blocked. Set allowDml=true to permit.`);
        }
        ...
        if (involved.length > 0 && opts.knownColumns.size > 0) {
            ...
            for (const ref of collectColumnRefs(node)) {
                if (ref.column === '*')
                    continue;
                if (!ref.table && selectAliases.has(ref.column.toLowerCase()))
                    continue;
                if (ref.table && cteNames.has(ref.table))
                    continue;
                ...
                if (ref.table) {
                    ...
                    if (candidates.length === 1) {
                        const correctAlias = candidates[0];
        ...
        if (warnings.some((w) => w.severity === 'error')) {
            throw new UnsafeSqlError(warnings
...
function hasStarColumn(node) {
    ...
    walk(node, (n) => {
        ...
        if (n &&
            ...
            const expr = n.expr;
            if (expr && expr.type === 'column_ref' && expr.column === '*')
                found = true;
        }
    });
    return found;
}
function hasLimit(node) {
...
function stripSchema(id) {
    const idx = id.indexOf('.');
    return idx >= 0 ? id.slice(idx + 1) : id;
}
function resolveColumnSet(tableRef, knownColumns) {
    if (knownColumns.has(tableRef))
        return knownColumns.get(tableRef) ?? null;
    // The known map is keyed by `schema.table` ids; lookup by bare name too.
    for (const [id, cols] of knownColumns) {
        if (id === tableRef || stripSchema(id) === tableRef)
            return cols;
    }
    return null;
}
 ...
 */
function buildAliasMap(node, knownTables, knownColumns, cteNames) {
    const out = new Map();
    walk(node, (n) => {
        if (!n || typeof n !== 'object')
            return;
        const obj = n;
        if ('column' in obj)
            return; // column_ref, not a FROM entry
        ...
        if (!knownTables.has(tableId) && !knownTables.has(stripSchema(tableId)))
            return;
...
function pickAlias(obj) {
    const as = obj.as;
    if (typeof as === 'string' && as)
        return as;
    if (as && typeof as === 'object' && 'value' in as) {
        const v = as.value;
...
function collectSelectAliases(node) {
    const out = new Set();
    walk(node, (n) => {
        if (!n || typeof n !== 'object')
            return;
        const obj = n;
        if (!('expr' in obj) || !('as' in obj))
            return;
        const alias = obj.as;
...
function extractColumnName(column) {
    ...
    if (!column || typeof column !== 'object')
        return null;
    const obj = column;
    if (obj.expr && typeof obj.expr.value === 'string')
 ...
 */
function buildAliasOwnerIndex(aliasMap) {
    // Identify unique tables by cols-set reference, remember preferred alias.
    const tablePreferredAlias = new Map();
    ...
    for (const [cols, preferredAlias] of tablePreferredAlias) {
        for (const c of cols) {
            ...
            if (!arr.includes(preferredAlias))
                arr.push(preferredAlias);
            out.set(lower, arr);
        }
    }
    return out;
 ...
 */
function rewriteAliasInSql(sql, wrongAlias, column, rightAlias) {
...
function walk(node, visit) {
    ...
    for (const v of Object.values(node)) {
        if (Array.isArray(v))
            v.forEach((x) => walk(x, visit));
        else if (v && typeof v === 'object')
            walk(v, visit);
    }
}
export function buildKnownSets(graph) {
 ...
 */
const knownSetsCache = new WeakMap();
export function buildKnownSetsMemoized(graph) {
