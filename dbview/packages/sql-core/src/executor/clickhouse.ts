import { createClient, type ClickHouseClient } from '@clickhouse/client';
import type { ExecuteQueryResponse } from '@dbview/shared';
import type { QueryExecutor } from './types.js';

export class ClickhouseExecutor implements QueryExecutor {
  private client: ClickHouseClient | undefined;

  constructor(private readonly connectionString: string) {}

  private getClient(): ClickHouseClient {
    if (!this.client) {
      this.client = createClient({
        url: this.connectionString,
        request_timeout: 5_000,
        application: 'dbview-executor',
        // Force server-side read-only mode (level 1 = no writes, no settings change).
        clickhouse_settings: { readonly: '1' },
      });
    }
    return this.client;
  }

  async run(sql: string, rowLimit: number): Promise<ExecuteQueryResponse> {
    const start = performance.now();
    const client = this.getClient();
    const rs = await client.query({ query: sql, format: 'JSONCompactEachRowWithNamesAndTypes' });
    const stream = await rs.json<unknown>();
    const arr = stream as unknown[];
    // First element = column names, second = column types, rest = rows.
    const columns = Array.isArray(arr[0]) ? (arr[0] as string[]) : [];
    const dataRows = arr.slice(2) as unknown[][];
    const truncated = dataRows.length > rowLimit;
    return {
      columns,
      rows: dataRows.slice(0, rowLimit),
      rowCount: Math.min(dataRows.length, rowLimit),
      durationMs: Math.round(performance.now() - start),
      truncated,
    };
  }

  async close(): Promise<void> {
    if (this.client) {
      await this.client.close();
      this.client = undefined;
    }
  }
}
