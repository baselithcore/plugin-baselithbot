...
export class MssqlIntrospector implements SchemaIntrospector {
  ...
  async introspect(): Promise<SchemaGraph> {
    try {
      ...
      for (const r of fksRes.recordset as Array<{
        src_schema: string;
        src_table: string;
        src_column: string;
      }>) {
      ...
      for (const row of colsRes.recordset as Array<{
        table_schema: string;
        table_name: string;
        column_name: string;
        data_type: string;
