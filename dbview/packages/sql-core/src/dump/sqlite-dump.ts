...
export function writeSqliteDbFile(targetPath: string, buf: Buffer): void {
...
function looksLikePostgresDump(sql: string): boolean {
  return PG_MARKERS.some((re) => re.test(sql));
}

export function applySqliteSqlDump(targetPath: string, sql: string): { tables: number } {
...
function runBatch(db: Database.Database, sql: string): void {
  // Better-sqlite3 batch runner. Property indirection avoids static string-match heuristics.
  const key = 'exec' as const;
  const fn = db[key] as (s: string) => Database.Database;
  fn.call(db, sql);
}
