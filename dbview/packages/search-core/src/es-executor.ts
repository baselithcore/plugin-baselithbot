...
export class ElasticsearchExecutor {
  ...
  async run(query: string, rowLimit: number): Promise<ExecuteQueryResponse> {
    ...
    if (env.op === 'indices') {
      const stats = (await client.cat.indices({ format: 'json' })) as unknown as Array<
    ...
    if (env.op === 'count') {
      const res = await client.count({
        index: env.index!,
        ...(env.query ? { query: env.query as Record<string, unknown> } : {}),
      });
    ...
    if (env.op === 'mapping') {
      ...
      for (const [idx, mapping] of Object.entries(res)) {
        const props = (mapping as { mappings?: { properties?: Record<string, unknown> } }).mappings
          ?.properties;
        if (props) {
          for (const [field, def] of Object.entries(props)) {
            const t = (def as { type?: string }).type ?? 'object';
    ...
    if (env.op === 'get') {
      ...
      const src = (res._source ?? null) as Record<string, unknown> | null;
      return finalize(
        ['_id', '_source'],
    ...
    if (env.op === 'mget') {
      ...
      const rows = (res.docs ?? []).map((d) => {
        ...
        const src = (d as { _source?: Record<string, unknown> })._source;
    ...
    if (body.size === undefined) body.size = rowLimit;
    const searchRes = await client.search({
      index: env.index!,
      ...body,
    });
    return shapeSearchResponse(searchRes as unknown, rowLimit, start);
  }
...
function shapeSearchResponse(res: unknown, rowLimit: number, start: number): ExecuteQueryResponse {
  const r = res as {
    hits?: {
      hits?: Array<{
        _id?: string;
  ...
  for (const h of hits) {
    if (h._source) for (const k of Object.keys(h._source)) keys.add(k);
    if (keys.size >= 12) break;
  }
  const columns = ['_id', '_score', ...keys];
  const rows = hits.map((h) => {
    const row: unknown[] = [h._id ?? '', h._score ?? null];
    for (const k of keys) {
      const v = h._source?.[k];
