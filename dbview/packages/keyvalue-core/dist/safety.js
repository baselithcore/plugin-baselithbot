 ...
 */
export function parseRedisCommand(input) {
 ...
 */
export function validateRedisCommand(input) {
    const parsed = parseRedisCommand(input);
    const warnings = [];
    if (!READ_COMMANDS.has(parsed.command)) {
        throw new Error(`Command '${parsed.command}' is not allowed. dbview only permits read-only Redis commands.`);
    }
    if (parsed.command === 'KEYS') {
        warnings.push('KEYS scans the entire keyspace and blocks Redis. Prefer SCAN with COUNT for production data.');
    }
