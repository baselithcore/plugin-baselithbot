import { z } from 'zod';
import { DialectSchema, DIALECT_META } from './dialect.js';
...
export function buildConnectionString(params) {
    switch (params.dialect) {
        ...
        case 'oracle': {
            ...
            const auth = params.username !== undefined
            ...
            if (params.sid && !params.serviceName)
        ...
        case 'falkordb': {
            ...
            const auth = params.username !== undefined || params.password !== undefined
                ? `${encodeURIComponent(params.username ?? '')}:${encodeURIComponent(params.password ?? '')}@`
                : '';
        ...
        case 'salesforce-data-cloud': {
            ...
            qs.set('clientSecret', params.clientSecret);
            if (params.dataspace)
                qs.set('dataspace', params.dataspace);
 ...
 * Server-side only — input contains plaintext clientSecret.
 */
export function parseSalesforceDataCloudConnection(cs) {
    if (!cs.startsWith('salesforce-data-cloud://')) {
        throw new Error('salesforce-data-cloud: connection string must start with salesforce-data-cloud://');
    }
    const qIdx = cs.indexOf('?');
    if (qIdx === -1) {
