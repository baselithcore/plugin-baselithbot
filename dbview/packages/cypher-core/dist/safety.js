...
const ALLOWED_FIRST_KEYWORDS = ['MATCH', 'OPTIONAL', 'WITH', 'CALL', 'UNWIND', 'RETURN'];
export class CypherSafetyValidator {
    validate(rawQuery, opts) {
        const stripped = stripComments(rawQuery)
            .trim()
            .replace(/;+\s*$/, '');
        if (!stripped)
            throw new UnsafeSqlError('Empty Cypher after stripping comments.');
        if (stripped.includes(';'))
            throw new UnsafeSqlError('Multiple statements not allowed.');
        const upper = stripped.toUpperCase();
        const tokens = tokenize(upper);
        const firstWord = tokens[0];
        if (!firstWord || !ALLOWED_FIRST_KEYWORDS.includes(firstWord)) {
            throw new UnsafeSqlError(`Cypher must start with ${ALLOWED_FIRST_KEYWORDS.join('/')}; got '${firstWord ?? '<empty>'}'.`);
        }
        if (!opts.allowWrites) {
 ...
 * generic engine 500.
 */
function assertProperTermination(query) {
    ...
    if (/\bUNION\b/.test(upper))
        return;
    if (/\bYIELD\b/.test(upper))
        return;
    throw new UnsafeSqlError('Read query must end with RETURN (or UNION/YIELD). Trailing WITH/ORDER BY/LIMIT alone is invalid.');
}
/**
 ...
 */
function assertNoUnknownProperties(query, schema) {
    const known = new Set();
    ...
    for (const m of refs) {
        const prop = m[2];
        if (prop && !known.has(prop))
            unknown.add(`${m[1]}.${prop}`);
    }
    if (unknown.size > 0) {
        throw new UnsafeSqlError(`Property reference(s) not in schema: ${[...unknown].join(', ')}. Use only properties listed in the schema; do not invent (e.g. createdAt, updatedAt) when none exists.`);
    }
}
function assertNoSchemaTypeLeak(query) {
    ...
    const TYPE_TOKEN = /:\s*(INTEGER|STRING|FLOAT|BOOLEAN|DATE|DATETIME|TIME|POINT|LIST|MAP|NUMBER)\b/;
    for (const m of propMaps) {
        const body = m[1] ?? '';
        if (TYPE_TOKEN.test(body)) {
            throw new UnsafeSqlError('Property map contains a schema type name (e.g. `{qty: INTEGER}`). Schema types are descriptive — use literal values or omit the filter.');
        }
    }
}
function collectLabels(query) {
    const out = new Set();
