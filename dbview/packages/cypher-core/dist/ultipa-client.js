 ...
 */
export async function createUltipaClient(connectionString) {
    ...
    if (target.username !== undefined)
        builder.username(target.username);
    if (target.password !== undefined)
        builder.password(target.password);
    if (target.defaultGraph)
        builder.defaultGraph(target.defaultGraph);
    if (target.useSSL)
        builder.tls({});
 ...
 */
export function releaseUltipaClient(connectionString) {
...
export async function evictUltipaClient(connectionString) {
    const entry = cache.get(connectionString);
    if (!entry)
        return;
    cache.delete(connectionString);
...
//# sourceMappingURL=ultipa-client.js.map