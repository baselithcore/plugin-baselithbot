      ...
      {data && (
        <>
          <section className="grid grid-cols-3">
            ...
            <MetaPanel
              title="Created"
              value={formatAbsolute(data.created_at)}
            />
          </section>
...
function CanvasWidgetView({
  ...
  return (
    ...
    >
      ...
      {widget.type === "text" && (
        <div className="info-block">{widget.content}</div>
      )}
      ...
      {widget.type === "list" && (
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div className="info-block">
            {widget.ordered ? "Ordered list" : "Unordered list"} with{" "}
            {widget.items.length} child widgets.
          </div>
