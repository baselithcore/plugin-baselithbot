...
| `DBVIEW_INTERNAL_HOST` | `127.0.0.1` | Host the supervisor dials the embedded Node API on, and the bind host it *asks* for — the vendored app ignores it and listens on every interface regardless, which is why cross-pod rendezvous needs no change here. A wildcard is normalised to loopback for dialling. |
